...
/**
 * Table class for t006_assetdepreciation
 */
class t006_assetdepreciation extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 't006_assetdepreciation';
		$this->TableName = 't006_assetdepreciation';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t006_assetdepreciation`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('t006_assetdepreciation', 't006_assetdepreciation', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// asset_id
		$this->asset_id = new DbField('t006_assetdepreciation', 't006_assetdepreciation', 'x_asset_id', 'asset_id', '`asset_id`', '`asset_id`', 3, 11, -1, FALSE, '`asset_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ListOfYears
		$this->ListOfYears = new DbField('t006_assetdepreciation', 't006_assetdepreciation', 'x_ListOfYears', 'ListOfYears', '`ListOfYears`', '`ListOfYears`', 2, 6, -1, FALSE, '`ListOfYears`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ListOfYears->Nullable = FALSE; // NOT NULL field
		$this->ListOfYears->Required = TRUE; // Required field
		$this->ListOfYears->Sortable = TRUE; // Allow sort
		$this->ListOfYears->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['ListOfYears'] = &$this->ListOfYears;

		// NumberOfMonths
		$this->NumberOfMonths = new DbField('t006_assetdepreciation', 't006_assetdepreciation', 'x_NumberOfMonths', 'NumberOfMonths', '`NumberOfMonths`', '`NumberOfMonths`', 16, 4, -1, FALSE, '`NumberOfMonths`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->NumberOfMonths->Nullable = FALSE; // NOT NULL field
		$this->NumberOfMonths->Required = TRUE; // Required field
		$this->NumberOfMonths->Sortable = TRUE; // Allow sort
		$this->NumberOfMonths->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['NumberOfMonths'] = &$this->NumberOfMonths;

		// DepreciationAmount
		$this->DepreciationAmount = new DbField('t006_assetdepreciation', 't006_assetdepreciation', 'x_DepreciationAmount', 'DepreciationAmount', '`DepreciationAmount`', '`DepreciationAmount`', 4, 14, -1, FALSE, '`DepreciationAmount`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// DepreciationYtd
		$this->DepreciationYtd = new DbField('t006_assetdepreciation', 't006_assetdepreciation', 'x_DepreciationYtd', 'DepreciationYtd', '`DepreciationYtd`', '`DepreciationYtd`', 4, 14, -1, FALSE, '`DepreciationYtd`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// NetBookValue
		$this->NetBookValue = new DbField('t006_assetdepreciation', 't006_assetdepreciation', 'x_NetBookValue', 'NetBookValue', '`NetBookValue`', '`NetBookValue`', 4, 14, -1, FALSE, '`NetBookValue`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getMasterFilter()
	{
		...
		// Master filter
		$masterFilter = "";
		if ($this->getCurrentMasterTable() == "t004_asset") {
			if ($this->asset_id->getSessionValue() != "")
				$masterFilter .= "`id`=" . QuotedValue($this->asset_id->getSessionValue(), DATATYPE_NUMBER, "DB");
			else
				return "";
		}
		return $masterFilter;
	...
	public function getDetailFilter()
	{
		...
		if ($this->getCurrentMasterTable() == "t004_asset") {
			if ($this->asset_id->getSessionValue() != "")
				$detailFilter .= "`asset_id`=" . QuotedValue($this->asset_id->getSessionValue(), DATATYPE_NUMBER, "DB");
			else
				return "";
		}
		return $detailFilter;
	...
	// Master filter
	public function sqlMasterFilter_t004_asset()
	{
		return "`id`=@id@";
	}

	// Detail filter
	public function sqlDetailFilter_t004_asset()
	{
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`t006_assetdepreciation`";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->ListOfYears->DbValue = $row['ListOfYears'];
		$this->NumberOfMonths->DbValue = $row['NumberOfMonths'];
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "t006_assetdepreciationlist.php";
		}
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "t006_assetdepreciationedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "t006_assetdepreciationadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("t006_assetdepreciationview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("t006_assetdepreciationview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "t006_assetdepreciationadd.php?" . $this->getUrlParm($parm);
		else
			$url = "t006_assetdepreciationadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("t006_assetdepreciationedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("t006_assetdepreciationadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("t006_assetdepreciationdelete.php", $this->getUrlParm());
	}

	// Add master url
	public function addMasterUrl($url)
	{
		if ($this->getCurrentMasterTable() == "t004_asset" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
			$url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_id=" . urlencode($this->asset_id->CurrentValue);
		}
		return $url;
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->ListOfYears->setDbValue($rs->fields('ListOfYears'));
		$this->NumberOfMonths->setDbValue($rs->fields('NumberOfMonths'));
	...
	public function renderEditRow()
	{
		...
		$this->ListOfYears->EditValue = $this->ListOfYears->CurrentValue;
		...
		$this->NumberOfMonths->EditValue = $this->NumberOfMonths->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->ListOfYears);
					$doc->exportCaption($this->NumberOfMonths);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->ListOfYears);
						$doc->exportField($this->NumberOfMonths);
